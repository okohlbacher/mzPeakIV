...
const SMALL = "http://localhost:4173/mzPeakIV/static/small.mzpeak";

test("?file=<url> auto-opens, no clicks (LOAD via deep link)", async ({ page }) => {
  await page.goto(`./?file=${encodeURIComponent(SMALL)}`);
  // small.mzpeak is non-imaging → terminal "No Imaging Data" (D-06).
  await expect(page.getByTestId("stage")).toHaveText(/^(Ready|No Imaging Data)$/, { timeout: 30000 });
